...
type oauthFlowPayload struct {
	...
	// InvitationCode 邀请码（与 aff 返佣码是两套体系）：OAuth 注册链接可携带
	// invitation_code，经 auth_flow payload 透传到回调阶段的用户创建事务。
	InvitationCode string `json:"invitation_code,omitempty"`
}
...
func findOrCreateOAuthUser(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, affiliateCode string, invitationCodeValue string) (*model.User, error) {
	user := &model.User{}
	...
	if genericProvider, ok := provider.(*oauth.GenericOAuthProvider); ok {
		...
		err := model.DB.Transaction(func(tx *gorm.DB) error {
			// 邀请码校验与消费必须和建号同事务：并发注册不能超发邀请码名额
			var invitationCode *model.InvitationCode
			if common.InvitationCodeOAuthRequired || invitationCodeValue != "" {
				invitationCodeResult, err := model.GetUsableInvitationCodeWithTx(tx, invitationCodeValue)
				if err != nil {
					return err
				}
				invitationCode = invitationCodeResult
			...
			if invitationCode != nil {
				if err := model.ConsumeInvitationCodeWithTx(tx, invitationCode, user.Id, user.Username); err != nil {
					return err
				}
			}
