...
> 分支：`dev/risk-control-group-scoping-20260426`
...
- 前端已有大量复用：`web/src/components/table/users/modals/EditUserModal.jsx:101` 等
...
| 前端入口 | `web/src/App.jsx:165`、`web/src/components/layout/SiderBar.jsx:191-195` |
| 前端页面 | `web/src/pages/Risk/index.jsx`（≈2000 行） |
...
### 6.4 前端 `web/src/pages/Risk/index.jsx`
...
- 前端：`web/src/pages/Risk/__tests__/*.test.jsx`（若仓库已有 vitest/jest 基础设施则沿用；若无则只在后端补 + 手测前端）—— 启动前先检查 `web/package.json` 看是否有 test 脚本；若无则 **不引入** 新测试框架，避免范围蔓延
...
| `web/src/components/channel/PressureCoolingEditor.jsx` | 渠道级覆盖编辑器 |
...
| `web/src/hooks/usage-logs/useUsageLogsData.jsx` | 搜索参数、展开详情 |
| `web/src/components/table/usage-logs/UsageLogsFilters.jsx` | Session ID 输入框 |
