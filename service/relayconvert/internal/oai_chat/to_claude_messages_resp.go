...
func buildClaudeUsageFromOpenAIUsage(oaiUsage *dto.Usage) *dto.ClaudeUsage {
	...
	cacheCreationInPrompt := cacheCreationTokens
	...
	if oaiUsage.PromptTokensDetails.CachedTokens > 0 || cacheCreationInPrompt > 0 {
		// OpenAI usage counts cached and cache-write tokens inside
		// prompt_tokens, while Claude semantics reports input_tokens
		// excluding both. Both counts are unadjusted prefixes and may overlap,
		// so clamp a negative remainder at zero. 坑点 #133：仅有 cached_tokens
		// 而无 cache_write_tokens 时同样要减，否则 input_tokens 虚高造成缓存双计费。
