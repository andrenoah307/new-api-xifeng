...
// 转换时必须扣除（即使没有 cache_write_tokens），否则下游按 anthropic 语义重复计费 cache。
func TestBuildClaudeUsageFromOpenAIUsageSubtractsCacheFromInput(t *testing.T) {
	...
	require.NotNil(t, usage)
	assert.Equal(t, 329, usage.InputTokens)
	assert.Equal(t, 236416, usage.CacheReadInputTokens)
	assert.Equal(t, 63, usage.OutputTokens)
}

func TestBuildClaudeUsageFromOpenAIUsageSubtractsCacheCreation(t *testing.T) {
	...
	assert.Equal(t, 200, usage.CacheReadInputTokens)
	assert.Equal(t, 300, usage.CacheCreationInputTokens)
}

func TestBuildClaudeUsageFromOpenAIUsageCacheCreationSplitTakesMax(t *testing.T) {
	oaiUsage := &dto.Usage{PromptTokens: 1000}
...
func TestBuildClaudeUsageFromOpenAIUsageFloorsAtZero(t *testing.T) {
...
func TestBuildClaudeUsageFromOpenAIUsageNoCacheUnchanged(t *testing.T) {
	oaiUsage := &dto.Usage{PromptTokens: 1000}
...
func TestBuildClaudeUsageFromOpenAIUsageNilReturnsNil(t *testing.T) {
