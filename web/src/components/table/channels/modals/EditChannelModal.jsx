...
const normalizeErrorFilterStringList = (values) =>
  Array.from(
    new Set(
...
const normalizeErrorFilterStatusCodes = (values) =>
  Array.from(
    new Set(
      (Array.isArray(values) ? values : [])
...
const normalizeErrorFilterRule = (rule = {}) => {
  ...
  return {
    status_codes: normalizeErrorFilterStatusCodes(rule.status_codes),
    message_contains: normalizeErrorFilterStringList(rule.message_contains),
    error_codes: normalizeErrorFilterStringList(rule.error_codes),
    action,
    rewrite_message: String(rule.rewrite_message || ''),
    replace_status_code:
      Number.isInteger(replaceStatusCode) && replaceStatusCode > 0
        ? replaceStatusCode
        : 200,
...
const normalizeErrorFilterRules = (rules) =>
                ...
                <Spin spinning={loading}>
                  <div className='p-2 space-y-3' ref={formContainerRef}>
                    ...
                    <ErrorFilterRulesEditor
                      value={inputs.error_filter_rules}
                      onChange={(value) =>
                        handleChannelSettingsChange(
                          'error_filter_rules',
                          normalizeErrorFilterRules(value),
                        )
                      }
