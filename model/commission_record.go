...
func GetRefundCommissionInfo(refund *TicketRefund) (*RefundCommissionInfo, error) {
	...
	var inviter User
	if err := DB.Select("id, username, aff_quota").
...
// 实际扣回额被夹到 min(请求额, 净返佣, 邀请人 aff 余额)，绝不把 aff 扣成负数。
func applyCommissionClawback(tx *gorm.DB, refund *TicketRefund, requested int) (*CommissionClawbackResult, error) {
	...
	var inviter User
	if err := lockForUpdate(tx).Select("id, aff_quota").
		Where("id = ?", inviterId).First(&inviter).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return result, nil
		}
	...
	if actual > inviter.AffQuota {
		actual = inviter.AffQuota
	}
	if actual <= 0 {
		return result, nil
	}
	...
	res := tx.Model(&User{}).
		Where("id = ? AND aff_quota >= ?", inviterId, actual).
